...
public class BlogController {
    ...
    @PutMapping
    public JsonResult<Boolean> create(@Check BlogCreateDTO blogCreateDTO, MultipartFile file) throws IOException {
    ...
    @PutMapping("/createHtml")
    public JsonResult<Boolean> createHtml(@Check BlogCreateDTO blogCreateDTO, MultipartFile file) throws IOException {
        ThrowUtils.throwIfTrue(file == null || StringUtils.isBlank(file.getOriginalFilename()), BlogError.FILE_IS_NULL);
        byte[] content = IoUtils.getContent(file.getInputStream());
        ThrowUtils.throwIfTrue(content == null, BlogError.CREATE_ERROR);
        String html = IStringUtils.newString(content);
        BlogDTO dto = new BlogDTO();
        dto.setId(UUID.randomUUID().toString());
        BeanUtils.copyProperties(blogCreateDTO, dto, "id", "content", "tags");
        dto.setTags(blogCreateDTO.getTags().split(","));
        dto.setContent(IStringUtils.getBytes(StringUtils.substringBetween(html, "<body class='typora-export'>", "</body>")));
        dto.setPublishTime(LocalDateTime.now());
        return blogServerFeign.createHtml(dto);
    }
    ...
    @PostMapping
    public JsonResult<Boolean> update(@Check BlogUpdateDTO blogUpdateDTO, MultipartFile file) throws IOException {
        ThrowUtils.throwIfTrue(file == null || StringUtils.isBlank(file.getOriginalFilename()), BlogError.FILE_IS_NULL);
    ...
    public JsonResult<Boolean> updateHtml(@Check BlogUpdateDTO blogUpdateDTO, MultipartFile file) throws IOException {
        ThrowUtils.throwIfTrue(file == null || StringUtils.isBlank(file.getOriginalFilename()), BlogError.FILE_IS_NULL);
        ...
        dto.setContent(IStringUtils.getBytes(StringUtils.substringBetween(html, "<body class='typora-export'>", "</body>")));
        dto.setPublishTime(TimeUtils.parse(TimeUtils.Format._2, blogUpdateDTO.getPublishTime()));
        return blogServerFeign.updateHtml(dto);
    }
    ...
    @GetMapping("/sync")
    public JsonResult<Boolean> sync(@RequestParam(required = false) String id, boolean isMarkDown) {
        if (StringUtils.isBlank(id)) {
            return blogServerFeign.sync("ALL", isMarkDown);
        } else {
            return blogServerFeign.sync(id, isMarkDown);
        }
    }
}
