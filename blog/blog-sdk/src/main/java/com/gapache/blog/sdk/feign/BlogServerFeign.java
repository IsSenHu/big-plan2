...
public interface BlogServerFeign {
    ...
    @PutMapping("/blog/createHtml")
    JsonResult<Boolean> createHtml(BlogDTO dto);
    ...
    @PostMapping("/blog/updateHtml")
    JsonResult<Boolean> updateHtml(@RequestBody BlogDTO blog);

    @PostMapping("/blog/page")
    JsonResult<PageResult<SimpleBlogDTO>> findAll(@RequestBody IPageRequest<BlogQueryDTO> iPageRequest);

    @PostMapping("/blog/sync/{id}/{isMarkDown}")
    JsonResult<Boolean> sync(@PathVariable("id") String id, @PathVariable("isMarkDown") boolean isMarkDown);
}
