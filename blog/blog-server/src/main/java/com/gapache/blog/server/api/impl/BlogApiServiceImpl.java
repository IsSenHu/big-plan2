...
public class BlogApiServiceImpl implements BlogApiService {
    ...
    @Override
    public Boolean create(BlogDTO blog, boolean isMarkDown) {
        ...
        return saveOrUpdate(blog, false, isMarkDown);
    }
    ...
    @Override
    public Boolean update(BlogDTO blog, boolean isMarkDown) {
        log.info("Update Blog:[id:{}, title:{}, introduction:{}, publishTime:{}, category:{}, tags:{}]",
                blog.getId(), blog.getTitle(), blog.getIntroduction(), blog.getPublishTime(), blog.getCategory(), Arrays.toString(blog.getTags()));
        return saveOrUpdate(blog, true, isMarkDown);
    }

    private boolean saveOrUpdate(BlogDTO blog, boolean update, boolean isMarkDown) {
        String content = IStringUtils.newString(blog.getContent());
        byte[] bytes = blog.getContent();
        BlogData data = new BlogData();
        Blog document = new Blog();
        BeanUtils.copyProperties(blog, document, "content");
        if (isMarkDown) {
            log.info("要解析的md为:{}", content);
            List<MarkdownItemDTO> items = ParseMarkdownUtils.parse(content);
            log.info("md解析结果:{}", JSON.toJSONString(items));
            MarkdownDTO markdownDTO = new MarkdownDTO();
            markdownDTO.setItems(items);
            bytes = ProtocstuffUtils.bean2Byte(markdownDTO, MarkdownDTO.class);

            StringBuilder contentBuilder = new StringBuilder();
            for (MarkdownItemDTO item : items) {
                Object c = item.getContent();
                if (c != null) {
                    contentBuilder.append(c.toString());
                }
            }
    ...
    @Override
    public void sync(String id, boolean isMarkDown) {
        Set<String> blogIds;
            ...
            stringRedisTemplate.execute((RedisCallback<Object>) connection -> {
                for (String blogId : blogIds) {
                    ...
                    if (isMarkDown) {
                        ...
                        document.setContent(IStringUtils.newString(contentData));
                    }
                    blogEsRepository.index(document);
                }
                return null;
