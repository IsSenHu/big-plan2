...
public class BlogServiceImpl implements BlogService {
    ...
    @Override
    public JsonResult<FullBlogDTO> get(String id, boolean isMarkDown) {
        ...
        redisTemplate.execute((RedisCallback<Object>) connection -> {
            ...
            if (isMarkDown) {
                MarkdownDTO md = ProtocstuffUtils.byte2Bean(bytes, MarkdownDTO.class);
                dto.setContent(md != null ? md.getItems() : null);
            } else {
                dto.setHtml(IStringUtils.newString(bytes));
            }
            return null;
        });
